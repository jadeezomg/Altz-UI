...
local GetPlayerMapPosition, GetPlayerFacing = C_Map.GetPlayerMapPosition, GetPlayerFacing
...
local function GetBearing(unit)
    ...
    px, py = C_Map.GetPlayerMapPosition(C_Map.GetBestMapForUnit("player"),"player")
    ...
    tx, ty = C_Map.GetPlayerMapPosition(C_Map.GetBestMapForUnit(unit),unit)
...
local Enable = function(self)
    ...
    if(range and range.insideAlpha and range.outsideAlpha) then
        ...
        frame:SetScale(aCoreCDB["UnitframeOptions"]["arrowsacle"])

        frame.arrow = frame:CreateTexture(nil, "OVERLAY")
        frame.arrow:SetTexture"Interface\\Addons\\AltzUI\\media\\Arrow"
